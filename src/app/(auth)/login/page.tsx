...
export default function LoginPage() {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // Memanggil fungsi dari NextAuth untuk login
    const result = await signIn("credentials", {
      email,
      password,
      redirect: false, // Kita atur manual agar bisa menangani error
    });
    ...
    <div className="flex min-h-screen items-center justify-center bg-gray-50 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-8 shadow-lg">
        <div className="text-center">
          <h2 className="mt-6 text-3xl font-bold tracking-tight text-gray-900">Login Sistem</h2>
          <p className="mt-2 text-sm text-gray-600">
            Penilaian Kinerja Pegawai Non-ASN <br />
            <span className="font-semibold text-blue-600">BBPVP Makassar</span>
          </p>
        </div>
